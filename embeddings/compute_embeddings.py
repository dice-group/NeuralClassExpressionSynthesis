...
def start(args):
    datasets = [Data(data_dir=f'../datasets/{f}/Triples/', train_plus_valid=args.train_plus_valid) for f in args.kbs]
    for i, d in enumerate(datasets):
        folder_name = args.kbs[i]
        experiment = Experiment(dataset=d,
                                model=args.model_name,
                                parameters=vars(args), ith_logger='_' + folder_name,
                                store_emb_dataframe=args.store_emb_dataframe, storage_path=f"../datasets/{folder_name}/Embeddings")
        print('Storage path: ', f"../datasets/{folder_name}/Embeddings")
        try:
